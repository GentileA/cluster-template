...
for i in range(6):
  ...
  node.addService(pg.Execute(shell="sh", command="sudo -H -u GentilA bash -c '/local/repository/ssh_setup.sh'"))
 
  node.addService(pg.Execute(shell="sh", command="sudo su GentilA -c 'cp /local/repository/source/* /users/GentilA'"))
